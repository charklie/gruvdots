...
local colorscheme_file =
  string.format("%s/.config/waybar/active_colorscheme.txt", home)

local file, err = io.open(colorscheme_file, "r")
assert(file, "Failed to open colorscheme file: " .. (err or "unknown error"))

local colorscheme_file_contents = file:read("*a")
file:close()

assert(colorscheme_file_contents, "Failed to read colorscheme file contents")
colorscheme_file_contents = colorscheme_file_contents:gsub("%s+", "")

if colorscheme_file_contents == "gruvbox_light" then
  vim.g.background = "light"
  vim.g.default_colorscheme = "gruvbox_light"
elseif colorscheme_file_contents == "gruvbox_dark" then
  vim.g.background = "dark"
  vim.g.default_colorscheme = "gruvbox_dark"
elseif
  colorscheme_file_contents ~= "gruvbox_dark"
  and colorscheme_file_contents ~= "gruvbox_light"
then
  vim.g.default_colorscheme = colorscheme_file_contents
end

print("Colorscheme set to:", vim.g.default_colorscheme)
